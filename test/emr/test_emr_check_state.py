...
def test_get_emr_cluster_state():
    assert check_state.get_emr_cluster_state(
        'Cluster-Match', emr_list_clusters=mocked_response) == MATCH_STATE


def test_get_emr_cluster_state_exception():
    with pytest.raises(NoSuchActivityError):
        check_state.get_emr_cluster_state(
            'Not Exist', emr_list_clusters=mocked_response)


@patch('aws_utils.utils.poller')
def test_poll_for_cluster_state(mocked_poller):
    mocked_poller.return_value = True
    assert check_state.poll_for_cluster_state(
        'Cluster-Match', REGION, MATCH_STATE, 0, 0) == True

    mocked_poller.return_value = False
    assert check_state.poll_for_cluster_state(
        'Cluster-Match', REGION, NOMATCH_STATE, 0, 0) == False
...
def test_poll_for_cluster_state_retry(mocked_get_boto3_emr_list_clusters, mocked_get_emr_cluster_state):
    ...
    assert check_state.poll_for_cluster_state(
        'Cluster-Match', REGION, MATCH_STATE, 0, 10) == True
    mocked_get_boto3_emr_list_clusters.assert_called_with(REGION)
    mocked_get_emr_cluster_state.assert_called_with(
        'Cluster-Match', mocked_response)

    assert check_state.poll_for_cluster_state(
        'Cluster-Match', REGION, NOMATCH_STATE, 0, 10) == False
    mocked_get_boto3_emr_list_clusters.assert_called_with(REGION)
    mocked_get_emr_cluster_state.assert_called_with(
        'Cluster-Match', mocked_response)
